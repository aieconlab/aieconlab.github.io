...
- **초고 단계(2026-07-27).** 본문 수치는 사전 검증 프롬프트(`PROMPT_trend16_intelligence_price.md`, 메인 저장소 루트·미추적)의 확정 사실(2장)을 따랐고, 그중 아래 '원자료 대장'으로 수집 가능한 항목은 이 세션에서 원문과 1:1 재대조했다(아래 대조 결과).
- **게시 직전 재확인(2026-07-28 06:30 KST 전후, 필수)**
  ...
  2. **Artificial Analysis 지수 완주 비용 4건** — 동적 렌더 사이트라 정적 사본에 수치가 없음. 계획 세션(2026-07-27) 확인값을 본문·그림 2에 사용했으므로 게시 직전 사이트에서 값 유지 여부 확인.
...
- **미확인으로 남긴 것**: WSJ 2026-04-28로 인용되는 'OpenAI 연환산 매출 2~4월 횡보' 원문(본문 '보도에 따르면' 처리), Muse Spark 1.1 단가(공식 발표문에 없음 — 본문 미사용), Kimi K3 라이선스(미공개 — 본문 미기재), Gemini 3.6 Flash 출시일(미확인 — 본문·그림은 '2026년 5월 이후 세대'로만 서술).
...
/opt/anaconda3/bin/python3 scripts/fig02_task_cost_slope.py      # 그림 2: 토큰 단가 vs 지수 완주 비용 기울기 그래프
...
- 게시본 SHA-256 (scripts/out/과 게시 경로 동일, 2026-07-27):
  - `fig01_output_price_moves.png` `462fab07dbd1d78b621b5f650aac0118009161546eaf3d298e74d99e7eb7a63a` (적대적 검수 반영: 구글 행을 '7월 신모델'에서 '2026-05~ 세대'로 교정해 재생성)
  - `fig02_task_cost_slope.png` `49d551fa57e83fbad8d9d382a4e53d6ef6622f72919c8763a44f4e3d1808eb66`
  - `trend16_cover.png` `5f0662a53279b2a5a2cc0162adc4acc669340bd8d313f55bda3a00b285e0c535`
- 출력 → 게시 경로 복사:
...
| artificialanalysis_models.html | https://artificialanalysis.ai/models (지수 완주 비용 수치는 동적 — 본문 값은 계획 세션 확인) | `edb540850be8d929666e0a1b5544239d4425f8f75821a3c7035570cdb8e6874c` |
...
- **미대조 잔여**: Artificial Analysis 완주 비용 4건(동적 — 게시 직전 재확인 항목), WSJ 보도 원문, DeepSeek 인하 공지 원문(2026-05-25 발표·05-31 발효는 계획 프롬프트 확정 항목 — 공식 뉴스 인덱스에는 별도 항목 없음).

## 핵심 수치 요약

| 수치 | 값 | 출처 |
|---|---|---|
| 출력 단가 스펙트럼 | $0.28(V4-Flash) ~ $50(Fable 5) = 178.6배(약 180배) | 각 사 공식 가격 문서 |
| 전작 대비 | GPT-5.6 Sol 동결($30), Grok 4.5 ×2.4, Kimi K3 ×3.75, Gemini 3.5F ×3.6(3.6F ×3.0), Fable 5 신설 | 동 |
| 7월 이전 인하 | Opus 4.1→4.5 $75→$25(2025-11-24), V4-Pro $3.48→$0.87(2026-05-31 발효) | Anthropic 문서·계획 프롬프트 |
| 완주 비용 | Sonnet 5 $4,010.12(300M) vs Opus 4.8(max) $3,752.55(120M) → 단가 60% 싼 쪽이 +6.9% | Artificial Analysis(2026-07-27 확인) |
| 급 낮춤 | GPT-5.6 Sol(low) $400.79(6.6M, 지능 49) = Opus 4.8의 1/9.4 | 동 |
| 사용량 | 분당 100억→160억 초과→약 190억→약 220억, 1조+ 고객 330→375 초과→거의 500 | 알파벳 콜·I/O·발표문 |
| 탄력성 | 가격 -10% → 사용량 +0.5~0.7% (≈ -0.05~-0.07) | arXiv 2601.10088 |
| 워크로드 | 프롬프트 1.5K→6K+(×4), 완성 150→400(×3), 시퀀스 2,000-→5,400+(2023말→2025말) | 동 |
